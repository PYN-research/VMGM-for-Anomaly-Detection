...
def adult_preprocessing(dir='/home/user/puyannan/Anomaly-Detection-with-Miss/ImAD-Anomaly-Detection-With-Missing-Data-main/data/Adult/adult.data', n_train=10000, n_test=2000, seed=42):
...
def get_missing_data(data_path, missing_rate, save_dir):
    ...
    num_test_data = 4000
    num_test_abnormal = 800
    train_data = normal_data[:num_train_data]#(12000,14)
    train_lab = np.zeros(len(train_data))
    test_normal_data = normal_data[num_train_data:num_train_data+num_test_data]#(4000,14)
    test_abnormal_data = abnormal_data[:num_test_abnormal]#(800,14)
    ...
    np.save(osp.join(save_dir, 'train_data.npy'), train_data[:12000,-1])
    np.save(osp.join(save_dir, 'train_labels.npy'), train_lab[:12000,-1])
    np.save(osp.join(save_dir, 'test_data.npy'), test_data[:4800,-1])
    np.save(osp.join(save_dir, 'test_labels.npy'), test_lab[:4800,-1])
...
def KDD_preprocess(num_normal=0, num_abnormal=0):
    ...
    file_names = [osp.join(path, "kddcup.data_10_percent"), osp.join(path, "kddcup.names")]
