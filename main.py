...
################################################################################
@click.command()
@click.option('--dataset_name', default='botnet', type=click.Choice(['kdd','botnet']))
@click.option('--load_arguments', type=click.Path(), default='botnet.json',
              help='arguments JSON-file path (default: None).')
@click.option('--repeat', type=int, default=5, help='the repeat time for calculating the average metric.')
...
#@click.option("--config", type=str, default="breast.yaml")
def main(dataset_name, load_arguments, repeat, stop_threshold, _lambda, rsd, seed, beta, 
            latent_dimension, missing_rate, entropy_reg_coe, mu, std, 
    ...
    print(f'Mum_cond_masks: {args.num_cond_masks}')
    ...
    for i in range(1, args.repeat + 1):
        ...
        model = ADMissingValue(
                ...
                split=args.split,
                mechanism=mechanism,
                num_cond_masks=args.num_cond_masks
                )
        results = model.train()
        AUROC.append(results['auroc'])
        AUPRC.append(results['auprc'])
    ...
    print('AUROC_Mean:',auroc_mean)
    print('AUROC_Std:',auroc_std)
    print('AUPRC_Mean:',auprc_mean)
    print('AUPRC_Std:',auprc_std)
